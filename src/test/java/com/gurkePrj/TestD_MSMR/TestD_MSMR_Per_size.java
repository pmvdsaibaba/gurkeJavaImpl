package com.gurkePrj.TestD_MSMR;
...
public class TestD_MSMR_Per_size {
    ...
    public void testProcAddR_test1() throws Exception {
        ...
        // Use a working copy of receiverStatesMap AND dkjRMap for all state updates
        Map<Integer, Map<Integer, SenderStateInReceiver>> currentReceiverStatesMap = new HashMap<>();
        Map<Integer, Map<Integer, byte[]>> currentDkjRMap = new HashMap<>();
        for (Map.Entry<Integer, d_MSMR.ReceiverEntry> entry : initResult.receiverStatesMap.entrySet()) {
            ...
            currentDkjRMap.put(entry.getKey(), new HashMap<>(entry.getValue().dkjRMap));
        }
        ...
        currentDkjRMap.put(newUid, newRcvResult.updatedState.dkjRMap);
        ...
        for (int r = 1; r <= nR; r++) {
            // Always use the latest receiver state and dkjRMap
            d_MSMR.ReceiverEntry receiverEntry = new d_MSMR.ReceiverEntry(
                false,
                currentReceiverStatesMap.get(r),
                currentDkjRMap.get(r)
            );
            ...
            currentDkjRMap.put(r, rcvResult.updatedState.dkjRMap);
        }
        ...
        for (int i = 0; i < newUids.length; i++) {
            ...
            // Insert new receiver state and dkjRMap for the new uid
            currentReceiverStatesMap.put(uid, new HashMap<>(addRes.newReceiverEntry.stateMap));
            currentDkjRMap.put(uid, new HashMap<>(addRes.newReceiverEntry.dkjRMap));
    ...
    public void testProcAddR_test2() throws Exception {
        ...
        for (Map.Entry<Integer, d_MSMR.ReceiverEntry> entry : initResult.receiverStatesMap.entrySet()) {
            ...
            currentReceiverStatesMap.put(
                entry.getKey(),
                new d_MSMR.ReceiverEntry(
                    entry.getValue().isNewAddRcvr,
                    new HashMap<>(entry.getValue().stateMap),
                    new HashMap<>(entry.getValue().dkjRMap)
                )
            );
        }

        currentReceiverStatesMap.put(newUid, new d_MSMR.ReceiverEntry(addResult.newReceiverEntry.isNewAddRcvr, new HashMap<>(addResult.newReceiverEntry.stateMap), new HashMap<>(addResult.newReceiverEntry.dkjRMap)));
    ...
    public void testProcRmvReceiver() throws Exception {
        ...
        Map<Integer, d_MSMR.ReceiverEntry> currentReceiverStatesMap = new HashMap<>();
        for (Map.Entry<Integer, d_MSMR.ReceiverEntry> entry : initResult.receiverStatesMap.entrySet()) {
            currentReceiverStatesMap.put(entry.getKey(), new d_MSMR.ReceiverEntry(entry.getValue().isNewAddRcvr, new HashMap<>(entry.getValue().stateMap), new HashMap<>(entry.getValue().dkjRMap)));
        }
    ...
    @RepeatedTest(1)
    public void testProcRmvSender() throws Exception {
        int nS = 7; // number of senders
        int nR = 15; // number of receivers
        int uidToRemove = 2; // Remove sender with ID 2

        // System.out.println("\n**********************************************");
        // System.out.println("* D_MSMR Remove Sender Test");
        // System.out.println("**********************************************");

        // Step 1: Initialize
        InitResult initResult = d_MSMR.procInit(nS, nR);
        SenderState senderState = initResult.senderStates.get(1); // sender performing the removal
        byte[] ad = new byte[16];
        new Random().nextBytes(ad);

        // Step 2: Remove a sender
        d_MSMR.RmvResult rmvResult = d_MSMR.procRmv(senderState, ad, d_MSMR.TargetType.SENDER, uidToRemove);
        assertNotNull(rmvResult.updatedSenderState, "Sender state should be updated after sender removal");
        assertNotNull(rmvResult.cR, "Receiver ciphertext (cR) should not be null after sender removal");
        assertNotNull(rmvResult.key, "Key should not be null after sender removal");
        assertNotNull(rmvResult.kid, "Kid should not be null after sender removal");

        // Verify the sender state was updated (member removed)
        assertFalse(rmvResult.updatedSenderState.memS.contains(uidToRemove), "Removed sender should not be in sender's memS");
        assertEquals(nS - 1, rmvResult.updatedSenderState.memS.size(), "memS should have one less member");
        ...
        // Test that all receivers can process the remove sender ciphertext
        for (int r = 1; r <= nR; r++) {
            d_MSMR.ReceiverEntry receiverEntry = currentReceiverStatesMap.get(r);
            assertNotNull(receiverEntry, "ReceiverEntry for " + r + " should exist");
            d_MSMR.ReceiveResult rcvResult = d_MSMR.procRcv(receiverEntry, ad, rmvResult.cR);
            assertNotNull(rcvResult, "Receiver " + r + " should be able to process remove sender ciphertext");
            assertTrue(rcvResult.success, "Receiver " + r + " should process remove sender ciphertext successfully");
            assertArrayEquals(rmvResult.key, rcvResult.key, "Receiver " + r + " keys should match after sender removal");
            currentReceiverStatesMap.put(r, rcvResult.updatedState);
        }
    ...
    @RepeatedTest(1)
    public void testProcAddS_test() throws Exception {
        int nS = 3; // initial number of senders
        int nR = 5; // number of receivers
        int newSenderUid = 10;

        // System.out.println("\n**********************************************");
        // System.out.println("* D_MSMR Add Sender Test ");
        // System.out.println("**********************************************");
        ...
        // --- Test: Newly added sender sends, all receivers should derive the same key ---
        // System.out.println("[Progress] Newly added sender (" + newSenderUid + ") sends, all receivers derive key:");
        // New sender sends
        d_MSMR.SendResult sendResult = d_MSMR.procSnd(addSResult.newSenderState, ad);
        assertNotNull(sendResult.ciphertext, "Ciphertext from new sender should not be null");
        assertNotNull(sendResult.key, "Key from new sender should not be null");

        // All receivers should be able to process and derive the same key
        for (int r = 1; r <= nR; r++) {
            d_MSMR.ReceiverEntry receiverEntry = initResult.receiverStatesMap.get(r);
            d_MSMR.ReceiveResult rcvResult = d_MSMR.procRcv(receiverEntry, ad, sendResult.ciphertext);
            assertNotNull(rcvResult, "Receiver " + r + " should be able to process new sender's ciphertext");
            assertTrue(rcvResult.success, "Receiver " + r + " should process new sender's ciphertext successfully");
            assertArrayEquals(sendResult.key, rcvResult.key, "Receiver " + r + " key should match new sender's key");
        }
        // System.out.println("All receivers derived the same key from the new sender's send operation.");
    ...
    @RepeatedTest(1)
    public void testProcAddR_multiSenderAdd() throws Exception {
        int nS = 5; // number of senders
        int nR = 10; // initial number of receivers

        // System.out.println("\n**********************************************");
        // System.out.println("* D_MSMR Multi-Sender Add Receiver Test");
        // System.out.println("**********************************************");

        // Step 1: Initialize
        InitResult initResult = d_MSMR.procInit(nS, nR);

        // Step 2: Sender 1 sends, all receivers generate a key
        SenderState sender1State = initResult.senderStates.get(1);
        byte[] ad = new byte[16];
        new Random().nextBytes(ad);


        // Use working copies for state updates
        Map<Integer, d_MSMR.ReceiverEntry> currentReceiverStatesMap = new HashMap<>();
        for (Map.Entry<Integer, d_MSMR.ReceiverEntry> entry : initResult.receiverStatesMap.entrySet()) {
            currentReceiverStatesMap.put(entry.getKey(), new d_MSMR.ReceiverEntry(entry.getValue().isNewAddRcvr, new HashMap<>(entry.getValue().stateMap), new HashMap<>(entry.getValue().dkjRMap)));
        }
        ...
        // Sender 1 sends
        SendResult sendResult1 = d_MSMR.procSnd(sender1State, ad);
        assertNotNull(sendResult1.ciphertext);
        assertNotNull(sendResult1.key);
        currentSenderStates.put(1, sendResult1.updatedState);

        // All receivers should be able to process and derive the same key
        for (int r = 1; r <= nR; r++) {
            d_MSMR.ReceiverEntry receiverEntry = initResult.receiverStatesMap.get(r);
            ReceiveResult rcvResult = d_MSMR.procRcv(receiverEntry, ad, sendResult1.ciphertext);
            assertNotNull(rcvResult, "Receiver " + r + " should be able to process sender 1's ciphertext");
            assertTrue(rcvResult.success, "Receiver " + r + " should process sender 1's ciphertext successfully");
            assertArrayEquals(sendResult1.key, rcvResult.key, "Receiver " + r + " key should match sender 1's key");
            currentReceiverStatesMap.put(r, rcvResult.updatedState);
        }
        ...
        for (int r = 1; r <= nR; r++) {
            ...
            assertNotNull(rcvResult, "Receiver " + r + " should be able to process add ciphertext for newUid1");
            assertTrue(rcvResult.success, "Receiver " + r + " should process add ciphertext for newUid1 successfully");
            assertArrayEquals(addResult1.key, rcvResult.key, "Receiver " + r + " key should match sender after add");
            // Update receiver state after receive
            currentReceiverStatesMap.put(r, rcvResult.updatedState);
        }
...
// Before this add that all senders first send and all receivers process. Else below add will fail
        ...
        // Step 4: Sender 2 adds another new receiver
        int newUid2 = 12;
    ...
    public static void printIntList(List<Integer> intList)
    {
        ...
        // @RepeatedTest(1)
        // public void testUBKEMPerformanceAllFunctions() throws Exception {
        //     // System.out.println("\n**********************************************");
        //     // System.out.println("* UB_KEM Performance Test (1000x each function)");
        //     // System.out.println("**********************************************");
        ...
        //     // Dummy key and state for testing
        //     byte[] pk = new byte[32];
        //     byte[] sk = new byte[32];
        //     byte[] ek = new byte[32];
        //     byte[] ciphertext = new byte[64];
        //     new java.util.Random().nextBytes(pk);
        //     new java.util.Random().nextBytes(sk);
        //     new java.util.Random().nextBytes(ek);
        //     new java.util.Random().nextBytes(ciphertext);

        //     // UB_KEM function wrappers
        //     for (int i = 0; i < iterations; i++) {
        //         UB_KEM.enc(pk, ad);
        //     }
        //     for (int i = 0; i < iterations; i++) {
        //         UB_KEM.dec(sk, ad, ciphertext);
        //     }
        //     for (int i = 0; i < iterations; i++) {
        //         UB_KEM.fin(ek, ad);
        //     }
        ...
        //    //  UB_KEM.printAndResetUBKEMStats();
        //     // System.out.println("UB_KEM performance test completed.");
        // }

    // ============================================================
    // SIZE CALCULATION HELPER METHODS FOR D_MSMR
    // ============================================================

    // Calculate size of SenderState
    private int calculateSenderStateSize(SenderState state) {
        int size = 0;
        size += 4; // int i
        size += 4; // int updateCount
        size += calculateSetSize(state.memS); // Set<Integer> memS
        size += calculateSetSize(state.memR); // Set<Integer> memR
        size += calculateTreeEKSize(state.ek); // TreeEK ek
        size += (state.ssk != null ? state.ssk.length : 0); // byte[] ssk
        size += (state.svk != null ? state.svk.length : 0); // byte[] svk
        size += (state.tr != null ? state.tr.length : 0); // byte[] tr
        size += calculateQueuedOperationsSize(state.ops); // Queue<QueuedOperation> ops
        return size;
    }

    // Calculate size of ReceiverEntry
    private int calculateReceiverEntrySize(ReceiverEntry entry) {
        int size = 0;
        size += 1; // boolean isNewAddRcvr
        // Map<Integer, SenderStateInReceiver> stateMap
        if (entry.stateMap != null) {
            for (Map.Entry<Integer, SenderStateInReceiver> e : entry.stateMap.entrySet()) {
                size += 4; // Integer key
                size += calculateSenderStateInReceiverSize(e.getValue());
            }
        }
        // Map<Integer, byte[]> dkjRMap
        if (entry.dkjRMap != null) {
            for (Map.Entry<Integer, byte[]> e : entry.dkjRMap.entrySet()) {
                size += 4; // Integer key
                size += (e.getValue() != null ? e.getValue().length : 0);
            }
        }
        return size;
    }

    // Calculate size of SenderStateInReceiver
    private int calculateSenderStateInReceiverSize(SenderStateInReceiver state) {
        int size = 0;
        size += 4; // int updateCount
        size += calculateSetSize(state.memS); // Set<Integer> memS
        size += calculateSetSize(state.memR); // Set<Integer> memR
        // Map<Integer, TreeDk> dkMap
        if (state.dkMap != null) {
            for (Map.Entry<Integer, TreeDk> e : state.dkMap.entrySet()) {
                size += 4; // Integer key
                size += calculateTreeDkSize(e.getValue());
            }
        }
        // Map<Integer, byte[]> kemDkMap
        if (state.kemDkMap != null) {
            for (Map.Entry<Integer, byte[]> e : state.kemDkMap.entrySet()) {
                size += 4; // Integer key
                size += (e.getValue() != null ? e.getValue().length : 0);
            }
        }
        // Map<Integer, byte[]> svkMap
        if (state.svkMap != null) {
            for (Map.Entry<Integer, byte[]> e : state.svkMap.entrySet()) {
                size += 4; // Integer key
                size += (e.getValue() != null ? e.getValue().length : 0);
            }
        }
        // Map<Integer, byte[]> trMap
        if (state.trMap != null) {
            for (Map.Entry<Integer, byte[]> e : state.trMap.entrySet()) {
                size += 4; // Integer key
                size += (e.getValue() != null ? e.getValue().length : 0);
            }
        }
    ...
    private int calculateCiphertextSize(Ciphertext ciphertext) {
        ...
        size += calculateObjectSize(ciphertext.cM); // Object cM (approximate)
        size += calculateQueuedCiphertextSize(ciphertext.cq); // Queue<QueuedCiphertext> cq
        size += (ciphertext.svkStar != null ? ciphertext.svkStar.length : 0); // byte[] svkStar
        size += (ciphertext.svkPrime != null ? ciphertext.svkPrime.length : 0); // byte[] svkPrime
        size += calculateToOperationSize(ciphertext.to); // ToOperation to
        size += (ciphertext.signature != null ? ciphertext.signature.length : 0); // byte[] signature
        return size;
    }

    // Helper: Calculate Set<Integer> size
    private int calculateSetSize(Set<Integer> set) {
        return (set != null ? set.size() * 4 : 0); // 4 bytes per Integer
    }

    // Helper: Calculate TreeEK size
    private int calculateTreeEKSize(TreeEK treeEK) {
        if (treeEK == null) return 0;
        int size = 0;
        // Map<Integer, byte[]> dataPk
        Map<Integer, byte[]> dataPk = treeEK.getDataPk();
        if (dataPk != null) {
            for (Map.Entry<Integer, byte[]> e : dataPk.entrySet()) {
                size += 4; // Integer key
                size += (e.getValue() != null ? e.getValue().length : 0);
            }
        }
        // Tree object (approximate based on node count)
        size += calculateTreeSize(treeEK.getTree());
        return size;
    }

    // Helper: Calculate TreeDk size
    private int calculateTreeDkSize(TreeDk treeDk) {
        if (treeDk == null) return 0;
        int size = 0;
        // Map<Integer, byte[]> dataSk
        Map<Integer, byte[]> dataSk = treeDk.getDataSk();
        if (dataSk != null) {
            for (Map.Entry<Integer, byte[]> e : dataSk.entrySet()) {
                size += 4; // Integer key
                size += (e.getValue() != null ? e.getValue().length : 0);
            }
        }
        size += 4; // Integer leaf
        // Tree object
        size += calculateTreeSize(treeDk.getTree());
        return size;
    }

    // Helper: Calculate Tree size (approximate)
    private int calculateTreeSize(Tree tree) {
        if (tree == null) return 0;
        int size = 0;
        size += 4; // int numLeaves
        size += 4; // int treeSize
        // Approximate node storage (each node has pk, sk, and metadata)
        // This is a rough estimate; adjust based on actual structure
        size += tree.getSize() * 100; // ~100 bytes per node estimate
        return size;
    }

    // Helper: Calculate Queue<QueuedOperation> size
    private int calculateQueuedOperationsSize(Queue<QueuedOperation> ops) {
        if (ops == null) return 0;
        int size = 0;
        for (QueuedOperation op : ops) {
            size += 1; // OpType enum
            size += 1; // TargetType enum
            size += 4; // int uid
            size += calculateObjectSize(op.diff); // Object diff
            size += (op.ekuidR != null ? op.ekuidR.length : 0); // byte[] ekuidR
        }
        return size;
    }

    // Helper: Calculate Queue<QueuedCiphertext> size
    private int calculateQueuedCiphertextSize(Queue<QueuedCiphertext> cq) {
        if (cq == null) return 0;
        int size = 0;
        for (QueuedCiphertext qc : cq) {
            ...
            size += 1; // TargetType t
            size += 4; // int uid
            size += calculateObjectSize(qc.cM); // Object cM
            size += calculateObjectSize(qc.cP); // Object cP
        }
        return size;
    }

    // Helper: Calculate ToOperation size
    private int calculateToOperationSize(ToOperation to) {
        if (to == null) return 0;
        int size = 0;
        size += 1; // OpType op
        size += 1; // TargetType target
        size += 4; // int uid
        return size;
    }

    // Helper: Estimate Object size (very rough approximation)
    private int calculateObjectSize(Object obj) {
        if (obj == null) return 0;
        if (obj instanceof byte[]) {
            return ((byte[]) obj).length;
        }
        // For other objects, return a rough estimate
        return 50; // Arbitrary estimate for complex objects
    }
}
